 ...
 const assetPath = (p) => {
   const clean = String(p || '').replace(/^\/?(assets\/)?/, '');
   const base = (window.__ASSET_BASE || 'assets/').replace(/\/?$/, '/');
   return base + clean;
 };

 const DEFAULT_CODIGO_PIX_COPIA_COLA = "00020101021226900014br.gov.bcb.pix2568pix.adyen.com/pixqrcodelocation/pixloc/v1/loc/936hXbvVSumZfEhVNQKf8g5204000053039865802BR5925MONETIZZE IMPULSIONADORA 6009SAO PAULO62070503***63041AD0";
 const DEFAULT_URL_IMAGEM_QRCODE = assetPath("img/qrcode.webp"); // pode ser sobrescrito via painel (PHP)
 
 const PRODUCT_INFO = { 
 name: "Kit 3 T-Shirts Oversized Classic Algodão", 
 originalPrice: 387.00, 
 price: 97.98, 
 image: assetPath("img/product.jpg"), 
...
const progress = Math.min((filledFields / totalFields) * 100, 100);
 ...
 setTimeout(() => {
 requestAnimationFrame(() => {
 // Separa leituras de layout do scroll/foco para reduzir forced reflow.
 ...
 setTimeout(() => {
 try { errorElement.focus({preventScroll: true}); } catch(e) {}
 }, 120);
 });
 });
 }, 80);
 ...
 const addPaymentInfoEventId = 'evt_addpaymentinfo_' + uniqueOrderId;
...
body: JSON.stringify({ id: uniqueOrderId, name: formData.name, email: finalEmail, phone: finalPhone, cpf: formData.cpf || '', cep: formData.cep || '', address: formData.address || '', number: formData.number || '', city: formData.city || '', value: PRODUCT_INFO.price, product: PRODUCT_INFO.name, product_id: PRODUCT_INFO.id })
}).catch(() => {});
} catch(e) {}
}
 ...
 if (isFormLocked || isSubmitting) return;
 try {
 if (typeof window.spaGoBack === 'function' && document.getElementById('spa-checkout-wrapper')) {
 window.spaGoBack();
 return;
 }
...
e("img", { src: assetPath("img/logo-small-120.webp"), srcSet: assetPath("img/logo-small-120.webp") + " 120w, " + assetPath("img/logo-small-180.webp") + " 180w, " + assetPath("img/logo-small.webp") + " 240w", sizes: "104px", alt: "Logo", className: "h-8 w-auto object-contain", onError: (ev) => { try { const img = ev.target; if(!img.dataset.fallback){ img.dataset.fallback='1'; img.src = assetPath("img/logo.webp"); } } catch(e) {} } }),
 ...
 e("div", { className: "w-24 h-24 bg-slate-100 rounded-xl overflow-hidden flex-shrink-0 border border-slate-100 shadow-inner", style: { width: "96px", height: "96px", flexShrink: 0 } }, e("img", { src: PRODUCT_INFO.image, className: "checkout-product-thumb", style: { width: "100%", height: "100%", objectFit: "cover", objectPosition: "center top", display: "block" }, alt: PRODUCT_INFO.name, loading: "eager", decoding: "async", onError: (ev) => { try { const img = ev.target; if(!img.dataset.fallback){ img.dataset.fallback='1'; img.src = "/" + String(PRODUCT_INFO.image || '').replace(/^\/+/, ''); } } catch(e) {} } })),
 ...
 e("span", { className: "text-[10px] bg-indigo-50 border border-indigo-100 text-indigo-800 font-semibold px-2.5 py-1 rounded-full flex items-center gap-1.5" }, e(Icons.Package, {className: "w-3 h-3"}), "Pagamento via PIX")
 )
 )
 ),
...
// Normaliza URLs relativas (ex: 'assets/img/qrcode.webp') para não quebrar em /c/ ou /checkout/
try {
if (effectiveQrUrl && typeof effectiveQrUrl === 'string') {
const isHttp = /^https?:\/\//i.test(effectiveQrUrl);
const isRoot = effectiveQrUrl.startsWith('/');
const isRelative = effectiveQrUrl.startsWith('../') || effectiveQrUrl.startsWith('./');
if (!isHttp && !isRoot && !isRelative) {
...
'send_to': 'AW-18305665723/aa-2CLfdvMwcELuV6ZhE',
...
e("div", {className: "w-12 h-12 bg-slate-100 rounded-md border border-slate-200 overflow-hidden flex-shrink-0", style: { width: "48px", height: "48px", flexShrink: 0 }},
e("img", { src: PRODUCT_INFO.image, alt: PRODUCT_INFO.name, className: "checkout-product-thumb", style: { width: "100%", height: "100%", objectFit: "cover", objectPosition: "center top", display: "block" }, loading: "lazy", decoding: "async", onError: (ev) => { try { const img = ev.target; if(!img.dataset.fallback){ img.dataset.fallback='1'; img.src = "/" + String(PRODUCT_INFO.image || '').replace(/^\/+/, ''); } } catch(e) {} } })
),
