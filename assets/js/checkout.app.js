...
if (typeof setupKeyboardDetection === 'function') setupKeyboardDetection();
 ...
 const DEFAULT_CODIGO_PIX_COPIA_COLA = "";
 const DEFAULT_URL_IMAGEM_QRCODE = "/assets/img/qrcode.webp"; // pode ser sobrescrito via painel (PHP)
 const PIX_CONFIG_ENDPOINT = "/api/pix-config";

 const normalizePixCode = (value) => typeof value === 'string' ? value.trim() : '';
 const isValidPixCode = (value) => {
...
const CHECKOUT_ACCENT_ORANGE = "#ff6b35";
const CHECKOUT_STEP_CIRCLE_STYLE = { backgroundColor: CHECKOUT_ACCENT_ORANGE, boxShadow: "0 4px 10px rgba(255, 107, 53, 0.24)" };
...
const CHECKOUT_GA_EVENT_PREFIX = '__ga_checkout_event__';
...
const getCheckoutGAContext = () => {
let visibleSource = 'direct';
try {
const wrapper = document.getElementById('spa-checkout-wrapper');
if (wrapper && window.getComputedStyle(wrapper).display !== 'none') visibleSource = 'spa';
} catch(e) {}
return {
checkout_entry_source: window.__checkoutEntrySource === 'lp' ? 'lp' : 'direct',
checkout_visible_source: visibleSource,
checkout_ready_state: document.readyState || ''
};
};

const claimCheckoutGAEvent = (eventName) => {
const openToken = getCheckoutOpenToken();
const memoryKey = eventName + '::' + openToken;
window.__checkoutTrackedGAEvents = window.__checkoutTrackedGAEvents || {};
if (window.__checkoutTrackedGAEvents[memoryKey]) return null;
window.__checkoutTrackedGAEvents[memoryKey] = true;

const eventId = window.generateEventId ? window.generateEventId() : 'ga_evt_' + Date.now();
writeSessionJson(CHECKOUT_GA_EVENT_PREFIX + memoryKey, { event_id: eventId, at: Date.now() });
return eventId;
};

const trackCheckoutGAEventOnce = (eventName, payload = {}) => {
if (isLabMode()) return false;
const eventId = claimCheckoutGAEvent(eventName);
if (!eventId) return false;
const gaPayload = { ...getCheckoutGAContext(), ...payload };
if (typeof window.trackGAEvent === 'function') {
try { window.trackGAEvent(eventName, gaPayload, eventId); } catch(e) {}
return true;
}
return false;
};

const getCheckoutDiagnosticContext = () => ({
...getCheckoutGAContext(),
checkout_scroll_viewport: getCheckoutScrollViewport() !== window ? 'spa_wrapper' : 'window'
});

const trackDiagnostic = (eventName, data = {}) => {
if (isLabMode()) return false;
try {
if (window.__izzatDiagnostics && typeof window.__izzatDiagnostics.track === 'function') {
window.__izzatDiagnostics.track(eventName, { ...getCheckoutDiagnosticContext(), ...(data || {}) });
return true;
}
} catch(e) {}
return false;
...
const trackDiagnosticOnce = (key, eventName, data = {}) => {
if (isLabMode()) return false;
try {
if (window.__izzatDiagnostics && typeof window.__izzatDiagnostics.trackOnce === 'function') {
return window.__izzatDiagnostics.trackOnce(key, eventName, { ...getCheckoutDiagnosticContext(), ...(data || {}) });
}
} catch(e) {}
return false;
};
...
const waitForCheckoutStylesReady = () => new Promise((resolve) => {
try {
if (window.__checkoutStylesReady) {
resolve();
return;
}
const css = document.querySelector('link[data-checkout-tailwind-css]');
if (!css) {
resolve();
return;
}
try {
if (css.sheet) {
window.__checkoutStylesReady = true;
resolve();
return;
}
} catch(e) {}
...
css.addEventListener('load', finish, { once: true });
css.addEventListener('error', finish, { once: true });
...
return { min: 4, max: 7 };
 ...
 Clock: ({className}) => e("svg", {className: className || "w-5 h-5", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round"}, e("circle", {cx: "12", cy: "12", r: "10"}), e("polyline", {points: "12 6 12 12 16 14"})),
...
if (eventSourceUrl && !source.event_source_url) source.event_source_url = eventSourceUrl;
 ...
 const getOrderDiagnosticsData = () => {
 const out = {};
 try {
 if (window.__izzatDiagnostics && window.__izzatDiagnostics.sessionId) {
 out.session_id = window.__izzatDiagnostics.sessionId;
 out.diagnostics_ready = true;
 } else {
 try {
 const savedSession = sessionStorage.getItem('izz_diag_session_id');
 if (savedSession) out.session_id = savedSession;
 } catch(e) {}
 out.diagnostics_ready = false;
 }
 } catch(e) {}
 try {
 const ua = navigator.userAgent || '';
 const lower = ua.toLowerCase();
 out.platform = /iphone|ipad|ipod/.test(lower) ? 'ios' : (/android/.test(lower) ? 'android' : (/windows/.test(lower) ? 'windows' : (/mac os/.test(lower) ? 'macos' : 'unknown')));
 out.browser_family = /tiktok|musical_ly|bytedance|aweme/.test(lower) ? 'tiktok_in_app' : (/instagram/.test(lower) ? 'instagram_in_app' : (/fbav|fban|facebook/.test(lower) ? 'facebook_in_app' : (/ wv\)|; wv|version\/4\.0 chrome\//.test(lower) ? 'android_webview' : (/crios|chrome/.test(lower) ? 'chrome' : (/safari/.test(lower) ? 'safari' : 'unknown')))));
 out.is_likely_tiktok_browser = /tiktok|musical_ly|bytedance|aweme/i.test(ua);
 } catch(e) {}
 try {
 const params = new URLSearchParams(window.location.search || '');
 out.is_tiktok_ad = !!params.get('ttclid') || /tiktok/i.test(params.get('utm_source') || '');
 } catch(e) {}
 try {
 const vv = window.visualViewport;
 out.page_path = window.location.pathname || '';
 out.checkout_entry_source = window.__checkoutEntrySource === 'lp' ? 'lp' : 'direct';
 out.checkout_scroll_viewport = getCheckoutScrollViewport() !== window ? 'spa_wrapper' : 'window';
 out.viewport_width = window.innerWidth || 0;
 out.viewport_height = window.innerHeight || 0;
 out.visual_viewport_width = vv ? Math.round(vv.width || 0) : 0;
 out.visual_viewport_height = vv ? Math.round(vv.height || 0) : 0;
 out.device_pixel_ratio = window.devicePixelRatio || 1;
 out.captured_at = new Date().toISOString();
 } catch(e) {}
 return out;
 };

 function CheckoutScreen({ onSuccess, pixReady, pixConfigError }) {
 ...
 // Segurança: lógica de tempo mantida para evitar ReferenceError (Crash)
 ...
 trackDiagnosticOnce('checkout_opened', 'checkout_opened', {
 source: window.__checkoutEntrySource === 'lp' ? 'lp' : 'direct'
 });
 trackDiagnosticOnce('checkout_app_loaded', 'checkout_app_loaded', {
 checkout_ready_state: document.readyState || ''
 });
...
const analyticsTimer = setTimeout(() => { if (!isLabMode() && window.loadAnalytics) window.loadAnalytics(); }, 3500);
 ...
 try {
 const state = window.__izzatDiagnostics && typeof window.__izzatDiagnostics.getCheckoutState === 'function'
 ? window.__izzatDiagnostics.getCheckoutState()
 : {};
 trackDiagnosticOnce('checkout_ready', 'checkout_ready', {
 checkout_ready_state: document.readyState || '',
 ...(state || {})
 });
 } catch(e) {}
 ...
 return () => { clearTimeout(analyticsTimer); clearTimeout(readyTimer); clearInterval(timerInterval); }
 }, []);
...
const progress = Math.min((filledFields / totalFields) * 100, 100);
 ...
 useEffect(() => {
 const root = document.getElementById('checkout-root') || formRef.current;
 const scrollViewport = getCheckoutScrollViewport();
 if (!root) return;

 const getScrollTop = () => {
 try {
 if (scrollViewport && scrollViewport !== window) return Math.max(0, Number(scrollViewport.scrollTop) || 0);
 return Math.max(0, Number(window.scrollY || window.pageYOffset || 0));
 } catch(e) { return 0; }
 };

 const getDiagnosticScrollMetrics = () => {
 try {
 if (window.__izzatDiagnostics && typeof window.__izzatDiagnostics.getScrollMetrics === 'function') {
 return window.__izzatDiagnostics.getScrollMetrics() || {};
 }
 } catch(e) {}
 return { scroll_top: Math.round(getScrollTop()) };
 };

 const trackInteraction = (interactionType, extra = {}) => {
 trackCheckoutGAEventOnce('CheckoutInteraction', {
 interaction_type: interactionType,
 ...extra
 });
 if (interactionType === 'scroll') {
 trackDiagnosticOnce('checkout_first_scroll', 'checkout_first_scroll', {
 interaction_type: interactionType,
 ...getDiagnosticScrollMetrics(),
 ...extra
 });
 } else if (interactionType === 'pointer' || interactionType === 'keyboard') {
 trackDiagnosticOnce('checkout_first_interaction', 'checkout_first_interaction', {
 interaction_type: interactionType,
 ...extra
 });
 }
 ...
 const getFieldMeta = (target) => {
 if (!target || !target.tagName) return null;
 const tag = String(target.tagName).toLowerCase();
 if (!/^(input|textarea|select)$/.test(tag)) return null;
 const type = String(target.type || tag).toLowerCase();
 if (type === 'hidden' || target.disabled) return null;
 return {
 field_name: String(target.name || target.id || target.getAttribute('autocomplete') || tag).slice(0, 80),
 field_type: type
 };
 };

 const trackFormFirstTouch = (target) => {
 const meta = getFieldMeta(target);
 if (!meta) return;
 trackCheckoutGAEventOnce('CheckoutFormFirstTouch', {
 interaction_type: 'field_focus',
 ...meta
 });
 trackDiagnosticOnce('checkout_first_field_focus', 'checkout_first_field_focus', {
 interaction_type: 'field_focus',
 ...meta
 });
 trackInteraction('field_focus', meta);
 };

 const onScroll = () => {
 trackInteraction('scroll', { scroll_top: Math.round(getScrollTop()) });
 };
 const onPointerDown = (event) => {
 const meta = getFieldMeta(event.target);
 if (meta) {
 trackFormFirstTouch(event.target);
 return;
 }
 ...
 const scrollTarget = scrollViewport && scrollViewport !== window ? scrollViewport : window;
 try { scrollTarget.addEventListener('scroll', onScroll, { passive: true }); } catch(e) {}
 try { root.addEventListener('pointerdown', onPointerDown, true); } catch(e) {}
 try { root.addEventListener('focusin', onFocusIn, true); } catch(e) {}
 try { root.addEventListener('keydown', onKeyDown, true); } catch(e) {}

 return () => {
 try { scrollTarget.removeEventListener('scroll', onScroll); } catch(e) {}
 try { root.removeEventListener('pointerdown', onPointerDown, true); } catch(e) {}
 try { root.removeEventListener('focusin', onFocusIn, true); } catch(e) {}
 try { root.removeEventListener('keydown', onKeyDown, true); } catch(e) {}
 };
 }, []);
...
const normalizeCepAddress = (provider, data) => {
 ...
 trackDiagnostic('api_error', {
 api_name: 'cep_lookup',
 error_name: 'cep_lookup_failed',
 error_message: e && e.message ? e.message : 'cep_lookup_failed'
 });
 } finally {
 ...
 // Correção 4: race condition check logo no início
 ...
 trackDiagnostic('api_error', {
 api_name: 'pix-config',
 error_name: 'pix_config_unavailable',
 error_message: pixConfigError || 'pix_config_not_ready'
 });
 trackEvent('Checkout_Error', {
 error_field: 'pix_config',
 error_message: 'PIX indisponivel',
 event_id: window.generateEventId()
 });
 return;
 }
 ...
 const orderDiagnostics = getOrderDiagnosticsData();
 fetch('/api/orders', {
 method: 'POST',
 headers: { 'Content-Type': 'application/json' },
 body: JSON.stringify({ id: uniqueOrderId, name: formData.name, email: finalEmail, phone: finalPhone, cpf: formData.cpf || '', cep: formData.cep || '', address: formData.address || '', number: formData.number || '', neighborhood: formData.neighborhood || '', complement: formData.complement || '', city: formData.city || '', value: 197.99, source: orderSource, diagnostics: orderDiagnostics })
 }).catch(() => {});
 } catch(e) {}
 }
 });
...
return !!formData.address || !!formData.neighborhood || !!cepFailed || cd.length === 8;
 ...
 const isSubmitDisabled = loading || isFormLocked || isSubmitting || !pixReady;
 const submitButtonText = !pixReady
 ? (pixConfigError ? "PIX indisponível. Recarregue" : "Preparando PIX...")
 : "CONFIRMAR A COMPRA";
 ...
 e("div", { className: "max-w-[480px] mx-auto p-4 pt-5 space-y-4 " },
...
e("div", { className: "absolute top-0 left-0 bg-green-600 text-white text-[10px] font-bold px-3 py-1 rounded-br-lg shadow-sm tracking-wide" }, "OFERTA TIKTOK"),
 e("div", { className: "w-24 h-24 bg-white rounded-xl overflow-hidden flex-shrink-0 border border-slate-100 p-2 shadow-inner" }, e("img", { src: PRODUCT_INFO.image, width: 96, height: 96, className: "w-full h-full object-contain transform group-hover:scale-105 transition-transform duration-500", alt: PRODUCT_INFO.name, loading: "eager", decoding: "async", onError: (ev) => { try { const img = ev.target; if(!img.dataset.fallback){ img.dataset.fallback='1'; img.src = "/" + String(PRODUCT_INFO.image || '').replace(/^\/+/, ''); } } catch(e) {} } })),
 ...
 e("span", { className: "text-[10px] bg-amber-50 border border-amber-100 text-amber-900 font-semibold px-2.5 py-1 rounded-full flex items-center gap-1.5" }, e(Icons.Package, {className: "w-3 h-3"}), "Garantia 12 Meses")
 )
 )
 ),
...
e("div", {className: "col-span-2"}, e("label", { className: "text-[10px] font-bold text-gray-400 uppercase pl-1 mb-1 block" }, "Cidade"), e("input", { name: "city", value: formData.city, onChange: handleChange, className: "w-full p-3.5 bg-white border border-slate-200 rounded-xl text-slate-600 text-base font-medium focus:border-green-500 outline-none", style: CHECKOUT_ADDRESS_INPUT_STYLE, placeholder: "Cidade/UF", disabled: isFormLocked || isSubmitting, autoComplete: "address-level2", autoCorrect: "off", spellCheck: "false" }))
 ...
 e("button", { ref: submitButtonRef, disabled: isSubmitDisabled, type: "submit", className: `w-full mt-6 bg-gradient-to-r from-green-600 to-green-700 text-white font-bold py-4 rounded-xl text-lg transition-all active:scale-[0.98] flex justify-center items-center gap-2 shadow-xl shadow-green-500/30 ${isSubmitDisabled ? 'opacity-80 grayscale cursor-not-allowed' : 'hover:shadow-green-500/50 hover:-translate-y-0.5'} btn-tactile min-h-[56px]`, "aria-busy": loading || (!pixReady && !pixConfigError) },
loading ? e("span", {className: "flex items-center gap-2"}, e("div", { className: "spinner-mobile" }), "Processando...") : e("span", {className: "flex items-center gap-2"}, submitButtonText)
 ),
 !pixReady && e("p", { className: `mt-3 text-center text-[11px] font-semibold ${pixConfigError ? 'text-red-500' : 'text-slate-400'}` }, pixConfigError ? "Não foi possível carregar o PIX do painel. Recarregue a página." : "Preparando pagamento PIX seguro."),
 )
 )
 ),
 ),
...
const effectivePixCode = normalizePixCode(pixCode);
const pixCodeReady = isValidPixCode(effectivePixCode);
...
trackDiagnosticOnce('checkout_pix_screen_loaded:' + transactionId, 'checkout_pix_screen_loaded', {
checkout_ready_state: document.readyState || ''
});
...
return () => { clearTimeout(step1); clearTimeout(step2); clearTimeout(step3); clearInterval(countdownInterval); };
}, [transactionId]);

const doCopy = async () => {
if (!pixCodeReady) {
trackDiagnosticOnce('checkout_pix_copy_blocked:' + transactionId, 'api_error', {
api_name: 'pix-config',
error_name: 'pix_code_unavailable',
error_message: 'copy_blocked_without_valid_pix'
});
return;
}
trackDiagnosticOnce('checkout_pix_copy_attempt:' + transactionId, 'checkout_pix_copy_attempt', {});
try {
...
const copyPixText = () => {
if (!pixCodeReady) {
trackDiagnosticOnce('checkout_pix_text_blocked:' + transactionId, 'api_error', {
api_name: 'pix-config',
error_name: 'pix_code_unavailable',
error_message: 'text_copy_blocked_without_valid_pix'
});
return;
}
triggerCopyFeedback();
...
const copyPixBtn = () => {
if (!pixCodeReady) {
trackDiagnosticOnce('checkout_pix_button_blocked:' + transactionId, 'api_error', {
api_name: 'pix-config',
error_name: 'pix_code_unavailable',
error_message: 'button_copy_blocked_without_valid_pix'
});
return;
}
triggerCopyFeedback();
...
const pixCountdownText = String(pixMinutes).padStart(2, '0') + ':' + String(pixSeconds).padStart(2, '0');
const pixProgress = Math.max(0, Math.min(100, (pixTimeLeft / (15 * 60)) * 100));
if (!pixCodeReady) {
return e("div", { className: "min-h-screen bg-[#f8fafc] font-sans safe-area-padding flex items-center justify-center px-4" },
e("div", { className: "w-full max-w-[420px] bg-white border border-red-100 rounded-2xl p-5 text-center shadow-sm" },
e("div", { className: "mx-auto mb-3 w-11 h-11 rounded-full bg-red-50 text-red-500 flex items-center justify-center" }, e(Icons.Info, { className: "w-6 h-6" })),
e("h1", { className: "text-lg font-extrabold text-slate-900 mb-2" }, "Pagamento indisponível"),
e("p", { className: "text-sm text-slate-500 leading-relaxed mb-4" }, "Não foi possível carregar o PIX configurado no painel. Recarregue a página para tentar novamente."),
e("button", { type: "button", onClick: () => window.location.reload(), className: "w-full min-h-[48px] rounded-xl text-white text-sm font-bold", style: { background: 'linear-gradient(90deg, #ff6b35 0%, #ff8c42 100%)' } }, "Recarregar")
)
);
}
...
e("div", {className: "sticky top-0 z-30 bg-white border-b border-slate-100 shadow-sm", style: { position: 'sticky', top: 0 }},
e("div", {style: { height: '4px', width: '100%', background: '#ffe1d2', overflow: 'hidden' }},
e("div", {style: { height: '100%', width: pixProgress + '%', background: 'linear-gradient(90deg, #ff6b35 0%, #ff8c42 100%)', transition: 'width 1000ms linear' }})
),
e("div", {className: "max-w-[480px] mx-auto px-4 py-2 flex items-center gap-2"},
e("div", {className: "w-8 h-8 rounded-full bg-orange-50 flex items-center justify-center flex-shrink-0", style: { color: CHECKOUT_ACCENT_ORANGE }},
e(Icons.Clock, {className: "w-5 h-5"})
),
e("div", {className: "leading-tight"},
e("div", {className: "text-[11px] text-slate-400 font-medium"}, "Expira em"),
e("div", {className: "text-base font-extrabold text-slate-900 tabular-nums"}, pixCountdownText)
)
)
),
e("div", {className: "max-w-[480px] mx-auto px-4 pt-6"},
...
e("button", {
onClick: copyPixBtn,
className: `w-full py-3.5 rounded-xl font-bold text-white text-sm flex items-center justify-center gap-2 transition-all active:scale-[0.98] min-h-[48px] pix-copy-subtle-pulse ${copyConfirmed ? 'pix-copy-confirmed' : ''} btn-tactile`,
style: {
...
false && e("div", {className: "bg-white rounded-xl border border-slate-200 p-3 mb-3"},
...
e("img", { src: PRODUCT_INFO.image, width: 48, height: 48, alt: PRODUCT_INFO.name, className: "w-full h-full object-contain", loading: "lazy", decoding: "async", onError: (ev) => { try { const img = ev.target; if(!img.dataset.fallback){ img.dataset.fallback='1'; img.src = "/" + String(PRODUCT_INFO.image || '').replace(/^\/+/, ''); } } catch(e) {} } })
),
...
e("img", { src: qrImageSrc, width: 190, height: 190, alt: "QR Code PIX", className: "w-full h-full object-contain", loading: "lazy", decoding: "async", onError: (ev) => { try { const img = ev.target; if (img.dataset.fallback === '1') return; img.dataset.fallback = '1'; if (effectiveQrUrl) img.src = effectiveQrUrl; } catch(e) {} } })
),
...
inset: '-8px',
borderRadius: '999px',
background: 'radial-gradient(circle, rgba(255,107,53,0.12) 0%, rgba(255,107,53,0.05) 46%, rgba(255,255,255,0) 74%)',
...
borderTopColor: CHECKOUT_ACCENT_ORANGE,
borderRightColor: '#ffbd8f',
...
e("h2", { className: "text-[21px] font-extrabold text-slate-800 tracking-tight leading-tight text-center max-w-full break-words" }, loadingTitle),
e("p", { className: "mt-4 text-[13px] text-slate-500 leading-[1.6] text-center max-w-[248px] mx-auto" }, "Estamos preparando a etapa de pagamento. Por favor, não feche esta página.")
 ...
 const [pixConfig, setPixConfig] = useState({ pixCode: DEFAULT_CODIGO_PIX_COPIA_COLA, qrCodeUrl: DEFAULT_URL_IMAGEM_QRCODE, loaded: false, error: '' });
 
 useEffect(() => {
 const skeleton = document.getElementById('skeleton-loader');
 if (skeleton) {
 waitForCheckoutStylesReady().then(() => {
 runAfterCheckoutPaint(() => {
 ...
 (async () => {
 let lastError = 'pix_config_unavailable';
 for (let attempt = 0; attempt < 3; attempt += 1) {
 try {
 const res = await fetch(`${PIX_CONFIG_ENDPOINT}?_=${Date.now()}&attempt=${attempt}`, { cache: 'no-store', headers: { accept: 'application/json' } });
 let cfg = null;
 try { cfg = res ? await res.json() : null; } catch(e) { throw new Error('pix_config_invalid_json'); }
 if (!res || !res.ok || !cfg || cfg.ok === false) throw new Error((cfg && cfg.error) || ('pix_config_http_' + (res && res.status)));
 const code = normalizePixCode(cfg.pix_code);
 if (!isValidPixCode(code)) throw new Error('pix_config_invalid_code');
 const nextQrCodeUrl = cfg.qrcode_url === null ? '' : (typeof cfg.qrcode_url === 'string' && cfg.qrcode_url.trim() ? cfg.qrcode_url.trim() : DEFAULT_URL_IMAGEM_QRCODE);
 if (!cancelled) setPixConfig({ pixCode: code, qrCodeUrl: nextQrCodeUrl, loaded: true, error: '' });
 return;
 } catch (err) {
 lastError = err && err.message ? err.message : 'pix_config_unavailable';
 if (attempt < 2) await wait(450 * (attempt + 1));
 }
 }
 if (!cancelled) {
 setPixConfig(prev => ({ ...prev, pixCode: '', loaded: true, error: lastError }));
 trackDiagnostic('api_error', {
 api_name: 'pix-config',
 error_name: 'pix_config_load_failed',
 ...
 const pixReady = isValidPixCode(pixConfig.pixCode);
 return screen === 'checkout' ? e(CheckoutScreen, { pixReady, pixConfigError: pixConfig.loaded && !pixReady ? pixConfig.error || 'pix_config_invalid' : '', onSuccess: (data) => {
 if (!pixReady) {
 trackDiagnostic('api_error', {
 api_name: 'pix-config',
 error_name: 'pix_screen_blocked',
 error_message: pixConfig.error || 'pix_config_not_ready'
 });
 return;
 }
 setCustomerData(data); scrollCheckoutViewportToTop('auto'); setScreen('pix');
 ...
 if (rootElement) {
 // Correção 5: evita race condition se o script for executado duas vezes
