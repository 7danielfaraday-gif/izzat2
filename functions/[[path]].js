...
export async function onRequest(context) {
    ...
    if (url.pathname.match(/\.(css|js|png|jpg|jpeg|gif|svg|ico|woff|woff2|ttf|map)$/i)) {
        ...
        // 1. Libera APENAS os arquivos usados pela Safe Page para todo mundo
        ...
        // 2. Para QUALQUER OUTRO arquivo estático, exige cookie de Humano
        const cookiesCheck = request.headers.get('Cookie') || '';
        if (cookiesCheck.includes('is_human=true')) {
            return env.ASSETS.fetch(request);
        }

        // 3. Bot tentando bisbilhotar recebe 404
