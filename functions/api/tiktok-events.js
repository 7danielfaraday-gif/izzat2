...
export const TIKTOK_EVENT_SOURCE_PRIMARY_HOST = 'izzatcasa.shop';
const TIKTOK_EVENT_SOURCE_ALLOWED_HOSTS = new Set([
  'oficial.izzatcasa.shop',
  'izzatcasa.shop',
]);
...
const USER_FIELDS = [
  'email', 'sha256_email',
  'phone_number', 'phone', 'sha256_phone_number', 'sha256_phone',
...
export function normalizeEventSourceUrl(value) {
  try {
    ...
    } else if (hostname === 'www.izzatcasa.shop') {
      base.hostname = 'izzatcasa.shop';
    } else {
...
export async function buildSafeUser(user) {
  ...
  const rawEmail = hasText(raw.sha256_email) ? raw.sha256_email : raw.email;
  if (hasText(rawEmail)) {
  ...
  const rawPhone = raw.sha256_phone_number || raw.sha256_phone || raw.phone || raw.phone_number;
  if (hasText(rawPhone)) {
  ...
  const rawExternalId = hasText(raw.sha256_external_id) ? raw.sha256_external_id : raw.external_id;
  if (hasText(rawExternalId)) {
    const normalizedExternalId = rawExternalId.trim().toLowerCase();
